...
func (s *Store) SaveListeningSession(session ListeningSession) error {
	_, err := s.DB.Exec(`
		INSERT INTO sessions (id, item_id, session_id, "current_time", duration, created_at)
		VALUES (?, ?, ?, ?, ?, datetime('now'))
	...
	`, lastSessionID, session.ItemID, session.SessionID, session.CurrentTime, session.Duration)
	if err != nil {
...
func (s *Store) GetLastSession() (ListeningSession, error) {
	...
	err := s.DB.QueryRow(`
		SELECT item_id, session_id, "current_time", duration, created_at
		FROM sessions WHERE id = ?
	`, lastSessionID).Scan(&ls.ItemID, &ls.SessionID, &ls.CurrentTime, &ls.Duration, &createdAt)
